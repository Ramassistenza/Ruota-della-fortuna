...
let gameMode = "standard"; // standard | express
...
/* ======================================================
   SCELTA MODALITÀ DI GIOCO (STANDARD / EXPRESS)
====================================================== */
...
modeStandard?.addEventListener("click", async ()=>{
  ...
  await startNewRound();   // ✅ NON initGame
});

modeExpress?.addEventListener("click", async ()=>{
  gameMode = "express";
  expressMode = false;
...
confirmSolve?.addEventListener("click", ()=>{
  ...
  if(norm(attempt) === norm(currentPhrase)){
    ...
    setTimeout(()=>{
      ...
      if(again){
        askGameMode();   // 🔥 RIAPRE LA SCELTA MODALITÀ
      }
